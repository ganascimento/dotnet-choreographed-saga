...
public class CancelOrderConsumer : BackgroundService
{
    ...
    public CancelOrderConsumer(IEmailService emailService, IConfiguration configuration, ILogger<CancelOrderConsumer> logger)
    {
    ...
    private async void Consume(object? sender, BasicDeliverEventArgs e)
    {
        try
        {
            ...
            await _emailService.SendCancelOrderAsync(data!.Id);
